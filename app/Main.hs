...
solutionTo 9 = product [a,b,c]
    where
        c = 1000 - a - b
        a = (f1 b) `div` (f2 b)
        b = head $ filter hasIntegerSolution [1..999]
        -- Deconstructed the system of eqs so that we don't need to iterate on all possible Pythagorean triples
        l = 1000
        l2 = 500 * 1000
        hasIntegerSolution x = (f1 x) `mod` (f2 x) == 0
        f1 x = l2 - (l * x)
        f2 x = l - x
...
main :: IO ()
main = putStrLn $ show $ solutionTo 9
